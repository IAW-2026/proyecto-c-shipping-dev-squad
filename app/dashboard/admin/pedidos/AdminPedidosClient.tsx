...
export default function AdminPedidosClient({ shipments: initialShipments }: Props) {
  ...
  return (
    <div style={{ width: "90%", maxWidth: 1400, margin: "0 auto", padding: "2rem 1rem" }}>
      ...
      <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} placeholder="Buscar por número de orden..."
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {paginated.length === 0 ? (
          ...
          paginated.map(s => <ShipmentCard key={s.id} shipment={s} onClick={selectShipment} showBuyer />)
        )}
      </div>
